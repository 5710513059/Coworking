...
$book = $_GET['ID'];
$meSQL = "SELECT booksa.*,uploadfile.* FROM booksa,uploadfile WHERE booksa.ID LIKE '$book%'";
      ...
      <!-- Header -->
        ...
        <center>
          ...
          <p><h3>ห้อง :<?php echo $meResult['Rooms']; ?></h3>
            ...
            <p><h3>เวลาจอง :<?php echo $meResult['Start_time' ]; ?>:00-<?php echo $meResult['End_time' ]; ?>:00</h3>
            
            <p><h3>จำนวนผู้ใช้ :<?php echo $meResult['Usercount']; ?></h3>
              <p><h3>หลักฐานการโอนเงิน :<a href="fileupload/<?=$meResult["fileupload"]?>" target="_blank"><?php echo $meResult["fileupload"];?></a></h3>

            <p><h3>สถานะ :<?php echo $meResult['Status']; ?></h3>
            
        </center>
        <center><p><h3><button class="w3-button w3-black w3-round-large" name="st1"><?php echo "<a href='status.php?ID=$meResult[ID]'>"?>อนุมัติ</button></h3></a>
       <h3><button class="w3-button w3-black w3-round-large" name="st1"><?php echo "<a href='status.php?ID=$meResult[ID]'>"?>ไม่อนุมัติ</button></h3></a></p></center></h1>
